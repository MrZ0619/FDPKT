...
def run_epoch(max_problem, pro_path, skill_path, batch_size, is_train, min_problem_num, max_problem_num,
    ...
    for i, data in tqdm(enumerate(data_loader), desc='Loading'):

        last_problem, last_skill, last_ans, next_problem, next_skill, next_ans, last_error_feedback, next_error_feedback, last_partial_score, next_partial_score, mask = data

        if is_train:
            predict = model(last_problem, last_skill, last_ans, next_problem, next_skill, next_ans, last_error_feedback, next_error_feedback, last_partial_score, next_partial_score)
    ...
    rmse = np.sqrt(np.mean((labels_np - outputs_np) ** 2))
